...
##### 3.1.1.1. 최량변수선택법(best subset selection)

전체 변수로 만들 수 있는 모든 유형의 변수조합을 만든 후 평가기준이 되는 통계량([3.1.2. 참조](#312-변수선택의-기준))을 바탕으로 가장 좋은 변수조합을 선택하는 방법. 이론적으로 구상이 가능한 모든 변수조합을 검토할 수 있다는 점에서 장점이 있지만 변수의 수에 따라 연산량이 기하급수적으로 늘어난다는 단점이 있다. 변수의 수가 p개인 경우 구상이 가능한 변수조합의 수는 아래와 같다.

```math
\sum_{i = 0}^p \binom p i = 2^p
```

ISL에서는 이론적으로 가장 간단하고 직관적인 변수선택 방법으로 소개하고 있으나 일반적인 통계학 서적에서는 변수선택법으로 다루지 않는다. 하지만, 전진선택, 후진소거, 단계적선택법이 공통적으로 가지고 있는 맹점이 최적의 변수조합을 보장하지 못한다는 점과 컴퓨팅 비용의 감소로 인해 연산 가능한 모형의 개수가 크게 늘어났다는 점을 고려했을 때에 의외로 응용할 여지가 많은 방법이다.

##### 3.1.1.2. 전진선택법(forward stepwise selection)
...
##### 3.1.1.3. 후진소거법(backward stepwise selection)
...
##### 3.1.1.4. 단계적선택법(hybrid stepwise selection)
...
##### 3.1.2.3. Mallow's $C_p$

예측오차 최소화를 이론적 배경으로 하는 통계량.

```math
C_p = \frac {SSE}{\hat \sigma^2} - (n - 2p)
```

$C_p$는 모델의 편향과 분산을 종합적으로 평가할 수 있는 지표이다. $C_p \approx p$이면 모형이 적절하게 선택되었다고 평가한다.

$C_p$의 정의를 다음과 같이 분해할 수 있다.

```math
C_p = \boxed{\frac{SSE}{\hat \sigma^2} - (n - p)}_\text{편향} + \boxed{p}_\text{분산}
```

SSE의 기댓값은 $\mathbb{E}[SSE] = (n - p)\sigma^2$이다. 따라서, 모형이 적절하게 적합되어 SSE가 이론과 일치하는 값을 가지면 편향에 해당하는 항이 0이 된다. 반면, 중요한 변수가 빠져 모형이 충분히 데이터의 변동을 설명하지 못하는 경우 SSE가 커지게 되어 편향에 해당하는 항이 $>0$이 된다.

분산에 해당하는 항 $p$는 $C_p$가 표준화된 예측오차를 추정하기 위해 설계된 통계량이라는 점을 이해할 필요가 있다. 예측오차를 표준화하는 과정에서 분산에 해당하는 $p \sigma^2$항을 $\sigma^2$으로 표준화했기 때문에 분산에 해당하는 값인 $p$만 남은 것이다.

따라서, 편향없이 **정상적으로** 모형이 가질 수 있는 분산만을 반영한 $C_p$의 값은 약 $p$가 된다.


##### 3.1.2.3. 수정된 결정계수($R^2$)
...
##### 3.2.2.2. 라쏘회귀(LASSO Regression, Least Absolute Shrinkage and Selection Operation)
...
### 3.3. 일반화 가법모형(General Additive Model, GAM)
...
##### 3.3.1.2. 역적합 알고리즘(Backfitting Algorithm)
